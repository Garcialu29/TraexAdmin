<?php 
	...
	class ObjetosModel extends Mysql
	{
		...
		public function selectObjeto(int $idparametro)
		{
			...
			$sql = "SELECT * FROM tbl_objetos WHERE Id_Objeto = $this->intIdparametro";
		...
		public function InsertObjeto(string $Nombre, string $descripcion, string $UsuarioBt, string $fecha, int $idrol){
			...
			$sql = "SELECT * FROM tbl_objetos WHERE Nombre_Objeto = '{$this->strNombre}' ";
			$request = $this->select_all($sql);

			if(empty($request))
			{
				$query_insert  = "INSERT INTO tbl_objetos(Nombre_Objeto,Descripcion,Creado_Por,Fecha_Creacion,Id_Rol) VALUES(?,?,?,?,?)";
		...
		public function selectroles()
		{
			...
			$sql = "SELECT Id_Rol FROM tbl_rol ";
			$request = $this->select_all($sql);
			return $request;
		}
		...
		public function updateObjeto(int $idparametro, string $nombre, string $descripcion){
			...
			$sql = "SELECT * FROM tbl_objetos WHERE Nombre_Objeto = '$this->strParametro' AND Id_Objeto != $this->intIdParametro";
			$request = $this->select_all($sql);

			if(empty($request))
			{
				$sql = "UPDATE tbl_objetos SET Nombre_Objeto = ?, Descripcion = ? WHERE Id_Objeto = $this->intIdParametro ";
		...
		public function deleteParametro(int $idparametro)
		{
			...
			$sql = "SELECT * FROM tbl_objetos WHERE Id_Objeto = $this->IntParametro and Id_Objeto < 6";
			$request = $this->select_all($sql);
			if(empty($request))
			{
				$sql = "DELETE  FROM  tbl_objetos WHERE Id_Objeto = $this->IntParametro ";
				$arrData = array(0);
		...
		public function selectObjetosR($contenido)
		{
			$sql = "SELECT * FROM tbl_objetos
					WHERE Id_Objeto LIKE '%$contenido%' OR
						  Id_Rol LIKE '%$contenido%' OR
						  Nombre_Objeto LIKE '%$contenido%' OR
						  Descripcion LIKE '%$contenido%' OR
						  Tipo_Objeto LIKE '%$contenido%' OR
