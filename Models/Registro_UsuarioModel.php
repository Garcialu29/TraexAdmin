       ...
       // private $intprimeringreso;
        ...
        public function insertUsuario(int $rol,string $nombre,  int $telefono,string $direccion,string $email, int $strStatus, string $password, string $fechavencimiento ){
           ...
           $this->strrolusuario = $rol;
            ...
            //SELECT * FROM tbl_usuarios where Correo_Electronico = 'ana@gmail.com' or Num_Identidad = 08012;
            
            if(empty($request)){
                $query_insert = "INSERT INTO tbl_usuarios (id_rol,nombre,Telefono,Direccion,Correo_electronico,contrasena,id_estado_usuario,fecha_vencimiento)
                    VALUES(?,?,?,?,?,?,?,?)";
        ...
        public function getParametroFechaVencimiento(){//obtenemos el valor del parametro de intentos de acceso al sistema
            ...
            $sql = "SELECT valor_parametro FROM tbl_parametros WHERE nombre_parametro = 'NUM_DIAS_VENCIMIENTO' ";//OBTENEMOS EL VALOR DEL PARAMETRO DE LA FECHA DE VENCIMIENTO
            $request = $this->select($sql);
            return $request;
        }
	}
