<?php 
	...
	class ParametrosModel extends Mysql
	{
		...
		public function selecParametro(int $idparametro)
		{
			...
			$sql = "SELECT * FROM tbl_parametros WHERE Id_Parametro = $this->intIdparametro";
		...
		public function InsertParametro(string $Nombre, string $descripcion, string $UsuarioBt, int $valor, string $fecha){
			...
			$sql = "SELECT * FROM tbl_parametros WHERE Nombre_Parametro = '{$this->strNombre}' ";
			$request = $this->select_all($sql);

			if(empty($request))
			{
				$query_insert  = "INSERT INTO tbl_parametros(Nombre_Parametro,Descripcion,Creado_Por,Valor_Parametro,Fecha_Creacion) VALUES(?,?,?,?,?)";
		...
		public function updateParametro(int $idparametro, string $nombre, string $descripcion, int $valor, string $UsuarioBt, string $fecha){
			...
			$this->strUsuarioBt = $UsuarioBt;
			$this->strfecha = $fecha;
			$sql = "SELECT * FROM tbl_parametros WHERE Nombre_Parametro = '$this->strParametro' AND Id_Parametro != $this->intIdParametro";
			$request = $this->select_all($sql);

			if(empty($request))
			{
				$sql = "UPDATE tbl_parametros SET Nombre_Parametro = ?, Descripcion = ?, Valor_Parametro = ?,Modificado_Por = ?, Fecha_Modificado = ? WHERE Id_Parametro = $this->intIdParametro ";
		...
		public function deleteParametro(int $idparametro)
		{
			...
			$sql = "SELECT * FROM tbl_parametros WHERE Id_Parametro = $this->IntParametro and Id_Parametro < 5";
			$request = $this->select_all($sql);
			if(empty($request))
			{
				$sql = "DELETE  FROM  tbl_parametros WHERE Id_Parametro = $this->IntParametro ";
				$arrData = array(0);
		...
		public function selectParametrosR($contenido)
		{
			$sql = "SELECT * FROM tbl_parametros
					WHERE Id_Parametro LIKE '%$contenido%' OR
						Nombre_Parametro LIKE '%$contenido%' OR
						Valor_Parametro LIKE '%$contenido%' OR
						Descripcion LIKE '%$contenido%' OR
						Creado_Por LIKE '%$contenido%' OR
						Modificado_Por LIKE '%$contenido%'";
			$request = $this->select_all($sql);
