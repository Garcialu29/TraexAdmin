...
function fntFechaP() {
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url + '/Paquetes/getPaqueteF/';
    request.open("GET", ajaxUrl, true);
    request.send();
    request.onreadystatechange = function () {
        if (request.readyState == 4 && request.status == 200) {
            ...
            if (objData.status) {
                ...
                document.querySelector('#Fecha_pedido').value = objData.data; //trae la fecha de vencimient
...
function fntPDF() {
    ...
    let  buscador = $('.dataTables_filter input').val();
     var win = window.open( base_url + '/Parametros/getParametrosR/'+buscador, '_blank');
     win.focus();
}
